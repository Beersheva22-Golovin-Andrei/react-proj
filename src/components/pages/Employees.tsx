...
function buildColumns (userData:UserData): GridColDef[] {
    ...
    if (userData && userData.role === 'admin'){
        columns.push({field: 'actions',type: 'actions',
        getActions: (params: GridRowParams) => {
          return [<GridActionsCellItem icon={<GridDeleteForeverIcon/>} onClick={()=>deleteFn(params.id)} label="Delete" />]
        }})
    }
    return columns;
}
...
const Employees: React.FC = () => {
    ...
    return <Box sx={{display:'flex', justifyContent: 'center'}}>
        ...
        <DataGrid columns={buildColumns(useSelectorAuth())} rows={employees}/>
        </Box>
    </Box>
}
